...
import os, requests, datetime, threading, random, json, time, websocket
...
def scraper(guildid, channelid):
    ...
    try:
        token = random.choice(tokensinguild)
         ...
         print(f"{datetime.datetime.now().strftime(f'{g}%H:%M:%S')}{r}    {c}[ERROR]     {g}->    {r}Failed no tokens in Guild {c}{token}{g}****{r}")
         exit(0)
